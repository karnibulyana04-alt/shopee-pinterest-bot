...
app.post('/api/schedule', async (req, res) => {
  ...
  if (!boardName || !imageUrl || !scheduledAt) {
    return res
      .status(400)
      .json({ error: 'boardName, imageUrl, scheduledAt wajib diisi' });
  }
...
cron.schedule('* * * * *', async () => {
  ...
  for (const post of due) {
    try {
      ...
      console.error(
        `❌ Gagal kirim ke Make.com (${post.id}):`,
...
app.listen(PORT, () => {
  ...
  console.log(`🔑 GROQ_API_KEY length: ${groqKey.length}`);
  console.log(
    `🪝 MAKE_WEBHOOK_URL set: ${webhook ? 'YES' : 'NO (belum diisi!)'}`
  );
  console.log(
